...
from torch.nn import Linear, ReLU, Sequential, Conv2d, MaxPool2d, Module, Flatten, Softmax, Dropout
...
class mriCNN(mriCNNBase):
    ...
    def __init__(self, con_out, con_kernel, con_padding, linear_1_in, linear_1_out):
        ...
        self.cnn_layers = Sequential(
            ...
            Conv2d(60, con_out, kernel_size=con_kernel, stride=(1, 1), padding=con_padding),
            Dropout(p=0.2),
            ReLU(inplace=True),
...
def plot_learning_curve(history, dataset_name, current_fold=None):
    ...
    if current_fold is None:
        ...
        plot_filepath = 'cnn_model_evaluation/Training_and_Test_Loss_' + dataset_name + '.pdf'
        ...
        plot_filepath = 'cnn_model_evaluation/Training_and_Test_Accuracy_' + dataset_name + '.pdf'
        pyplot.savefig(plot_filepath)
        pyplot.close()
        ...
        plot_filepath = 'cnn_model_tuning/Training_and_CV_Loss_Fold_' + str(current_fold) + '_' + dataset_name + '.pdf'
        pyplot.savefig(plot_filepath)
        pyplot.close()
        ...
        pyplot.xlabel('Epoch')
        pyplot.ylabel('Accuracy')
        plot_filepath = 'cnn_model_tuning/Training_and_CV_Accuracy_Fold_' + str(current_fold) + '_' + dataset_name + '.pdf'
        pyplot.savefig(plot_filepath)
        pyplot.close()
...
def objective(params):
    ...
    for train, valid in kfold.split(inputs, targets):
        ...
        model = mriCNN(params["con_out"], params["con_kernel"], params["con_padding"], linear_1_in,
                       params["linear_1_out"])
    ...
    print('CNN Filter Size: ' + str(params['con_out']) + ' CNN Kernel Size: ' + str(params["con_kernel"]) +
          ...
          ' Epochs: ' + str(params['epochs']) + ' TPE Loss: ' + str(tpe_loss))
...
def k_fold_cross_validation(params):
    # Make sure parameters that need to be integers are integers
    for parameter_name in ['con_out', 'con_kernel', 'padding', 'linear_1_out', 'batch_size']:
        params[parameter_name] = int(params[parameter_name])
    ...
    for train, valid in kfold.split(inputs, targets):
        ...
        model = mriCNN(params["con_out"], params["con_kernel"], params["padding"], linear_1_in,
                       params["linear_1_out"])
    ...
    plot_filepath = 'cnn_model_tuning/Training_and_CV_Loss_Aggregate_' + params['dataset_name'] + '.pdf'
    pyplot.savefig(plot_filepath)
    pyplot.close()
    ...
    plot_filepath = 'cnn_model_tuning/Training_and_CV_Accuracy_Aggregate_' + params['dataset_name'] + '.pdf'
    pyplot.savefig(plot_filepath)
    pyplot.close()
